...
def main():
    """
    법안 표결 데이터를 수집, 처리하고 서버로 전송하는 메인 함수
    """
    notifier = Notifier()
    job_name = "법안 표결 데이터 업데이트"
    ...
    try:
        ...
        df_vote, df_vote_party = manager.update_bills_vote()
...
"
            f"법안 표결 데이터 업데이트가 성공적으로 완료되었습니다.
"
...
"
        ...
        if df_vote is not None:
            success_message += f"처리된 표결 데이터 수: {len(df_vote)}건\n"
        else:
            success_message += "처리된 표결 데이터가 없습니다.\n"

        if df_vote_party is not None:
            success_message += f"처리된 정당별 표결 데이터 수: {len(df_vote_party)}건"
        else:
            success_message += "처리된 정당별 표결 데이터가 없습니다."
